...
export function scheduleUpdateOnFiber(
  ...
  // 判断当前任务是否被打断
  // 如果有则用interruptedBy标记打断其他任务的Fiber
  checkForInterruption(fiber, expirationTime);
  recordScheduleUpdate();

  // TODO: computeExpirationForFiber also reads the priority. Pass the
  // priority as an argument to that function and this one.
  // 首次渲染优先级是 NormalPriority = 97
  const priorityLevel = getCurrentPriorityLevel();

  if (expirationTime === Sync) {
    // 首次渲染 expirationTime === Sync
    if (
      // Check if we're inside unbatchedUpdates
      // 首次渲染 executionContext 在unBatchUpdates的时候赋值为8 0b001000
...
function markUpdateTimeFromFiberToRoot(fiber, expirationTime) {
  ...
  if (fiber.expirationTime < expirationTime) {
    // fiber.expirationTime = 0  expirationTime === Sync === 1073741823
    // 首次渲染 更新fiber的过期时间
    fiber.expirationTime = expirationTime;
  }
  // 创建fiber的副本，生成双缓存Fiber树
  ...
  if (node === null && fiber.tag === HostRoot) {
    root = fiber.stateNode; // root 是FiberRoot
  } else {
    while (node !== null) {
  ...
  if (root !== null) {
    // 首次渲染workInProgressRoot = null !== root
    if (workInProgressRoot === root) {
      // Received an update to a tree that's in the middle of rendering. Mark
      // that's unprocessed work on this root.
      // 收到树的更新，标记节点未处理的更新
      // workInProgressRootNextUnprocessedUpdateTime === 0 === NoWork
      markUnprocessedUpdateTime(expirationTime);
      // workInProgressRootNextUnprocessedUpdateTime = expirationTime = Sync
...
function performConcurrentWorkOnRoot(root, didTimeout) {
  ...
  if (expirationTime !== NoWork) {
    ...
    if (workInProgress !== null) {
      // unBatchUpdates时 executionContext => 8
      const prevExecutionContext = executionContext;
      // executionContext => 24 => 0b11000
...
function performSyncWorkOnRoot(root) {
  ...
  // 首次渲染 lastExpiredTime是0
  const lastExpiredTime = root.lastExpiredTime;
  // 首次渲染 expirationTime = Sync = 1073741823
  ...
  // If the root or expiration time have changed, throw out the existing stack
  // and prepare a fresh one. Otherwise we'll continue where we left off.
  // 首次渲染 workInProgressRoot => null, renderExpirationTime => 0, expirationTime => Sync
...
// 创建双缓存
// 给workInProgressRootExitStatusXXX属性赋值
// renderExpirationTime => Sync
function prepareFreshStack(root, expirationTime) {
  root.finishedWork = null;
  root.finishedExpirationTime = NoWork;

  // 首次渲染 timeoutHandle = -1
  ...
  // renderExpirationTime渲染过期时间 => Sync
...
export function flushPassiveEffects() {
  // 首次渲染 pendingPassiveEffectsRenderPriority === NoPriority 不执行
